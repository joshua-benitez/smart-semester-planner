...
import { SubmissionNoteModal } from './SubmissionNoteModal'
...
export const AssignmentCard = ({ assignment, onDelete, index = 0, isSelected = false, onSelect, onStatusUpdate }: AssignmentCardProps) => {
  ...
  const handleSubmissionNoteSubmit = (note: string) => {
    if (!onStatusUpdate) return
    ...
    onStatusUpdate(assignment.id, 'completed', extras)
  }
  ...
  return (
    <>
      ...
      <SubmissionNoteModal
        ...
        onSubmit={handleSubmissionNoteSubmit}
        defaultValue={assignment.submissionNote || ''}
