              ...
              {parseDiagnostics && (
                <div className="mt-3 rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-xs font-medium text-gray-600">
                  {parseDiagnostics.source === 'ai'
                    ? `Gemini extraction${parseDiagnostics.model ? ` (${parseDiagnostics.model})` : ''}${parseDiagnostics.fallbackUsed ? ' + fallback' : ''}`
                    : `Local parser${parseDiagnostics.aiAvailable ? ' fallback' : ' (no Gemini key configured)'}`}
                </div>
              )}
